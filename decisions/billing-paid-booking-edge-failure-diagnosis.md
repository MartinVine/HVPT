...
Steps 1–2 above describe *completing* the `payg_class` product. **Per the §7 decision, that product is being abandoned in favour of the `credit_topup` route (option C).** With that direction, the durable fix instead means: (a) the client must stop sending the unsupported type and route to the proven `credit_topup` checkout, (b) it must **close the booking loop** after top‑up succeeds (top‑up grants credits but books nothing — §7C), and (c) steps 3–4 still apply — capacity/duplicate integrity on the eventual booking, and loud failure on unsupported types so the next contract drift is visible. The point that survives either direction: a fix that merely silences this member's error without restoring a working "no‑credit → booked" path does **not** address the defect class.

---

## 7. Recommended fix options (ranked — DO NOT IMPLEMENT here)

> **Decision (2026‑06‑18, Martin): pursue the `credit_topup` direction — option C below.** The intended product direction is to route no‑credit booking through the proven "buy a credit, then book" path rather than completing the per‑class `payg_class` product. The ranking below is annotated to reflect that; option A is retained for the record but is **not** the chosen path. The step‑2 fix task should be scoped against **option C**.
>
> All require **at least an edge‑function and/or client change**; none require a migration unless noted. A new task should be scoped after this diagnosis is reviewed.

- **C. Route no‑credit booking through the working `credit_topup` path *(CHOSEN — client‑led, possibly client‑only)*.** Make "no credit → book" offer "buy a credit, then book" instead of per‑class payment. `credit_topup` is already live, proven, and used daily (§3c), so the heavy lifting is on the **client** (`hvpt-app`): present the top‑up purchase, and — critically — **after the top‑up succeeds, complete the booking of the specific class the member was trying to book.** **Risk: low technically**, but note the two product/UX implications below.
  - **Pricing changes** from "£10 for *this* class" (`class_types.payg_rate`) to "buy a class credit" at the member's plan rate (`TOPUP_PRICE_MAP`: 8→£6.13, 12→£5.75, 16→£5.00, 20→£4.80 per credit) or the £10 PAYG fallback for members with no/unknown plan. Confirm this pricing is acceptable to Harry.
  - **Top‑up does not auto‑book.** The `credit_topup` webhook branch grants credits but does **not** book any class. So the fix must close the loop in the app: on return from a successful top‑up checkout, re‑attempt the original class booking (now that credits exist), and handle the race where the class filled up in the meantime (capacity 8) or the member abandons mid‑checkout. Without this, the member buys a credit but still isn't booked — a different bad outcome.
  - **Decommission the dead path:** because `payg_class` is being abandoned, consider whether the unused `create-checkout-session` "Unknown type" path and the never‑fired `payg_class` webhook handler should be left as‑is, or have a clear guard/log so the dead contract is documented rather than silently rotting. At minimum the client must stop sending the unsupported type.

- **A. Complete the `payg_class` branch in `create-checkout-session`** *(NOT chosen — kept for the record; edge‑function change only).* Implement the sender half to match the existing webhook receiver: look up the class + `payg_rate`, create a customer‑attached one‑time Checkout session with `metadata.type = "payg_class"` and `class_id/class_name/class_date`. **Risk: low–medium.** Self‑contained and additive, but builds out a per‑class payment product that Martin has elected not to pursue, and relies on a webhook path that has never run in production.

- **B. Auth‑aware PAYG via `create-payg-checkout`** *(NOT chosen; client + edge change).* Adapt the website PAYG function to accept an authenticated member and emit `payg_class`. **Risk: medium**, touches two repos; superseded by the decision above.

- **D. (Stopgap, independent of direction) Friendly client‑side error.** Until option C lands, have the app detect this path and show "online booking for paid classes isn't available yet — contact Harry" instead of the raw edge error. **Risk: low.** Does **not** restore the ability to book/pay; explicitly a holding measure.
